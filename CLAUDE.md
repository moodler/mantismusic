...
- `preview/` — Self-contained deployable directory (generated by build script)
  - `preview/discography.json` — Compiled music database
  - `preview/discography.js` — Same data as `window._discographyData` for `file://` protocol support
  - `preview/index.html` — Generated with OG tags and cache-busting hashes
  - `preview/js/`, `preview/css/` — Copied from source
  - `preview/music` — Symlink to parent directory for audio/image access

**Build Pipeline (`build_music_json.py`):**
- Reads music folder structure (tracks/, collections/, artist/)
- Prepares `site/` directory: copies js/, css/
...
- Generates root `index.html` with OpenGraph/Twitter meta tags and cache-busting hashes
- Generates per-track (`track/{slug}/index.html`) and per-collection (`collection/{id}/index.html`) OG pages with item-specific meta tags for social media sharing
...
- PREVIEW_DIR = DATA_DIR / 'preview' (self-contained deployable output)
...
- Preview button builds then opens `preview/index.html` via `file://` URL
- Build & Deploy button builds then rsyncs `preview/` directory (with `--copy-links` to follow symlinks)
...
2. `build_music_json.py` prepares `site/` and compiles discography.json, RSS feed, feed pages, index.html, and per-item OG pages
3. Frontend loads the JSON (via fetch or pre-loaded `window._discographyData` for file:// access) and renders the player
4. Audio files are accessed relative to the data directory root
5. Deploy rsyncs with `--copy-links` to follow symlinks and copy actual files

**Routing (app.js):**
- Uses History API (`pushState`/`popstate`) for clean URLs: `/track/{slug}`, `/collection/{id}`, `/collections`, `/about`
- Falls back to hash routing (`#/track/{slug}`) on `file://` protocol
- Old hash-style URLs are auto-redirected to clean paths on http/https
- `updatePath(path)` and `getCurrentPath()` abstract over both modes
- OG pages use `<base href="../../">` so the SPA's relative asset paths resolve correctly from 2 levels deep
...
├── index.html            # Generated root page with generic OG tags and cache-busting hashes
├── track/                # Generated per-track OG pages (track/{slug}/index.html)
├── collection/           # Generated per-collection OG pages (collection/{id}/index.html)
└── site/                 # Generated by build script — deployable assets
    ├── discography.json  # Compiled music data
    ├── discography.js    # Same data for file:// protocol support
    ├── js/, css/         # Copied from source
    ├── og-image.jpg      # Artist profile for generic OG tags
    ├── feed.rss          # RSS feed
...
- Deploy uses rsync with `--copy-links` to follow symlinks and `--exclude raw-*` to skip originals
- OG pages (`track/` and `collection/` directories) are full copies of index.html with item-specific meta tags and `<base href="../../">` for correct relative path resolution
